# Fixture: consumed artifact matches only by basename — directory identity lost
#
# Regression for basename matching: 02-slice consumes
# `unrelated/01-intent.md@runtime:01-intent`, but 01-intent declares
# `changes/{slice-id}/01-intent.md`. Same basename, different canonical path.
# Must fail — artifact identity is the full declared path.
...
kind: internal
purpose: p
inputs: unrelated/01-intent.md@runtime:01-intent
