# Fixture: inputs with provenance outside the declared classes
#
# `legacy@X` uses an unrecognized provenance class; `orphan-note` has no
# @provenance tag at all. Both must fail.
...
kind: internal
purpose: p
inputs: raw-request@E | legacy@X | orphan-note
